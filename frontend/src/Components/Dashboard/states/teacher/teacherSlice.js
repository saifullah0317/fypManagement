...
export const getTeachers = createAsyncThunk(
    ...
    async (thunkAPI) => {
        try {
            const resp = await axios.get('http://localhost:8080/getteachers');
...
const post = async (teacher) => {
    try {
        const resp = await axios.post('http://localhost:8080/postteacher', teacher);
...
const updtTeacher = async (id, data) => {
    try {
        await axios.put(`http://localhost:8080/updateteacher/${id}`, data);
    } catch (error) {
        console.log('something went wrong');
    }
}
